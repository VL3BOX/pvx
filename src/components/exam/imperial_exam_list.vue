<template>
    <div class="m-imperial-list">
        <div class="m-search">
            ...
            <div class="u-tip" v-if="!search">{{ $t('请输入关键字开始搜索，搜索词长度不少于') }}<span>{{ $t('2个字符') }}</span>。</div>
            <div class="u-tip" v-else>{{ $t('当前搜索关键字“') }}<span>{{ search }}</span>{{ $t('”，返回结果') }}<span>{{ total }}</span>{{ $t('条。') }}</div>
        </div>
